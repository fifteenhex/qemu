 ...
 *
 * Device tree "sstar,infinity-rtcpwc", reg = <0x1f006800 0x200>. The RTC
 * counter lives in an always-on power domain isolated from the digital (CPU)
 * domain; the driver reads it across the boundary with a DIG2RTC/RTC2DIG
 * "ISO" handshake:
 *
 ...
 *     RTC2DIG_ISO_CTRL_ACK (0x20 bit3) for the RTC domain to acknowledge, the
 *     expected ack ALTERNATING clear/set/clear/set/clear/set - i.e. the ack is
 *     the parity of the ISO_CTRL bits (6.5 drivers/sstar/rtc/ms_rtcpwc.c
 *     ms_rtc_ISOCTL()).
 *   - then it pulses CNT_RD (0x04 bit0) / CNT_RD_TRIG (0x38 bit0), waits for
 *     CNT_UPDATING (0x2c bit0) to clear, and reads the 32-bit seconds count
 *     from RDDATA_CNT_L/H (0x30/0x34): run_sec = (H << 16) | L.
 *
 * Without the handshake answered the vendor RTC driver fails to bring the RTC
 * up ("RTCPWC fail to enter correct state") and MainUI's power management
 * decides the system is unpowered and reboots. Model it just enough to read a
 * running wall clock: answer the ISO ack (parity), report the count as
 * valid/never-updating, and return a live seconds counter from the virtual
 * clock. Register 0x48 in the same bank is a power-source status the firmware
 * polls (bit2 = external DC power present); report it present.
 *
...
/* Power-source status in the same bank; bit2 = external power present */
...
{
    MStarRtcpwcState *s = MSTAR_RTCPWC(opaque);
    ...
    case RTCPWC_PWRSRC:
        /*
         * Power-source status: report running on battery, no external power.
         * (The bit only matters to the firmware's power management.)
         */
        return 0;
    default:
...
static const VMStateDescription vmstate_mstar_rtcpwc = {
    ...
    .fields = (const VMStateField[]) {
        VMSTATE_UINT16_ARRAY(regs, MStarRtcpwcState, MSTAR_RTCPWC_NUM_REGS),
...
static const TypeInfo mstar_rtcpwc_types[] = {
    {
...
DEFINE_TYPES(mstar_rtcpwc_types)
