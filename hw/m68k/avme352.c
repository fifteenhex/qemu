...
static uint32_t avme352_boot_sp, avme352_boot_pc;
...
static void avme352_init(MachineState *machine)
{
    ...
    if (machine->ram_size != AVME352_RAM_SIZE) {
        error_report("avme352: the card has a fixed 512 KiB of local RAM");
        exit(1);
    }
    memory_region_add_subregion(sysmem, AVME352_RAM_BASE, machine->ram);

    memory_region_init_ram(dpram, NULL, "avme352.dpram", AVME352_DPRAM_SIZE,
                           &error_fatal);
    memory_region_add_subregion(sysmem, AVME352_DPRAM_BASE, dpram);

    memory_region_init_rom(rom, NULL, "avme352.rom", AVME352_ROM_SIZE,
                           &error_fatal);
    memory_region_add_subregion(sysmem, AVME352_ROM_BASE, rom);

    glue = qdev_new(TYPE_AVME352_GLUE);
    /* Must be parented before anything can link to its GPIO inputs. */
    object_property_add_child(OBJECT(machine), "glue", OBJECT(glue));
    AVME352_GLUE(glue)->cpu = cpu;
    AVME352_GLUE(glue)->dpram = memory_region_get_ram_ptr(dpram);
    ...
    for (i = 0; i < AVME352_NSCC; i++) {
        ...
        qdev_prop_set_chr(scc, "chrA", serial_hd(i * 2));
        qdev_prop_set_chr(scc, "chrB", serial_hd(i * 2 + 1));
        ...
        object_property_add_child(OBJECT(machine), name, OBJECT(scc));
        /* Both channels of a chip share one request line. */
        sysbus_connect_irq(SYS_BUS_DEVICE(scc), 0,
                           qdev_get_gpio_in(glue, i));
        sysbus_connect_irq(SYS_BUS_DEVICE(scc), 1,
                           qdev_get_gpio_in(glue, i));
        sysbus_realize_and_unref(SYS_BUS_DEVICE(scc), &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(scc), 0, AVME352_SCC_BASE + i * 4);
        AVME352_GLUE(glue)->scc[i] = ESCC(scc);
    }

    sbd = SYS_BUS_DEVICE(glue);
    sysbus_realize_and_unref(sbd, &error_fatal);
    sysbus_mmio_map(sbd, 0, AVME352_GLUE_BASE);
    sysbus_mmio_map(sbd, 1, AVME352_DBSET_BASE);
    sysbus_mmio_map(sbd, 2, AVME352_DBCLR_BASE);
    /* The mailbox overlays the top of the dual ported RAM. */
    sysbus_mmio_map_overlap(sbd, 3, AVME352_DPRAM_BASE + AVME352_MBOX_OFF, 1);
    sysbus_mmio_map(sbd, 4, AVME352_VME_BASE);

    fw = machine->firmware ?: "avme352.bin";
    rombuf = NULL;
    if (!g_file_get_contents(fw, (gchar **)&rombuf, &romlen, NULL)) {
        ...
        if (!path ||
            ...
            error_report("avme352: could not read ROM image '%s'", fw);
            exit(1);
        }
        g_free(path);
    }
    ...
    avme352_boot_sp = ldl_be_p(rombuf);
    avme352_boot_pc = ldl_be_p(rombuf + 4);
    rom_add_blob_fixed("avme352.rom", rombuf, romlen, AVME352_ROM_BASE);
    g_free(rombuf);
}

static void avme352_reset(MachineState *machine, ResetType type)
{
    CPUM68KState *env = &M68K_CPU(first_cpu)->env;

    qemu_devices_reset(type);

    /*
     * The 68020 fetches the initial stack pointer and program counter from
     ...
     */
    env->aregs[7] = avme352_boot_sp;
    env->pc = avme352_boot_pc;
    env->sr = 0x2700;
    m68k_switch_sp(env);
}
...
static void avme352_machine_init(MachineClass *mc)
{
    ...
    mc->reset = avme352_reset;
    mc->default_cpu_type = M68K_CPU_TYPE_NAME("m68020");
    mc->max_cpus = 1;
    mc->no_floppy = 1;
    mc->no_cdrom = 1;
    mc->no_parallel = 1;
    mc->default_ram_size = AVME352_RAM_SIZE;
    mc->default_ram_id = "avme352.ram";
}
