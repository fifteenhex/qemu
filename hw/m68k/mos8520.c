...
static void mos8520_reset(DeviceState *dev)
{
    ...
    /*
     * Port A comes up as the board wants it; the silicon resets to all
     * inputs (0), but a machine that enters an OS directly, skipping
     * the boot ROM, uses these to model the port state the firmware
     * would have left behind (see the Amiga overlay line, CIA-A PA0).
     */
    s->pra = s->reset_pra;
    s->ddra = s->reset_ddra;
    s->prb = s->ddrb = 0;
...
static const Property mos8520_properties[] = {
    ...
    DEFINE_PROP_UINT8("reset-pra", MOS8520State, reset_pra, 0),
    DEFINE_PROP_UINT8("reset-ddra", MOS8520State, reset_ddra, 0),
};
