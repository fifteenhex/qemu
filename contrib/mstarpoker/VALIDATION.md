...
| 2026-07-21 | **DDR root cause (IPL disasm)** | disassembled the vendor IPL (`/tmp/ipl.bin`, Thumb): (1) DDR init is interleaved with ~15 timer-based delays (routine @IPL 0x1cd4 busy-waiting the PM timer 0x1f006050, r0=0x2ee0/0x4b0 ticks) that the write-only capture dropped; (2) more decisively, the PHY training phase (IPL 0x3200+) is read-modify-write and data-dependent (282 loads vs 188 stores; `ldrh;and #0xff00;orr;strh`, polls `ldrh;cmp #0x506` and `ldrh;ands #2;beq`, field branches `ldrh;ubfx #5,#2;cmp;bne`). It reads DQS/read-level/gate-training results from the PHY and computes the next writes from them | CONCLUSION: a flattened write list can never train real DRAM - the captured values are QEMU-stub-PHY artifacts and the control-flow path differs on silicon. `ddr_seq_ssd202d` corrected to say so. Faithful HW DDR bring-up requires *running* the IPL training code on-target (execute), not replaying writes |
