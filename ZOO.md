...
| `sun3x` (Sun 3/80) | 68030 (68851) | ✅ Linux shell · ⧗ **genuine 3/80 PROM boots real SunOS 4.1.1: root mounts off the UFS miniroot (`root on sd6a`), the kernel recognizes the Sun-3/80, prints its ethernet address, and enters device autoconfiguration** (`sm0 at obio`, `st0-3`/`sr0` SCSI probes) — **the kernel completes FULL device autoconfiguration** (`sm0 st0-3 sr0 sd0-sd6 fdc0 fd0 zs0 zs1 le0`), no crash/panic; banked at wall M8 (the DVMA resource-map `rmalloc` allocator spins waiting for interrupt-driven DMA completion to refill the ~64 KB DVMA arena — a separate subsystem). Fixed en route: sd-open SCSI handshake, an 82072 FDC responder, a memory-error-register status mask, and a real level-7-NMI storm | `hw/m68k/sun3x-NOTES.md`, `hw/m68k/sun3x-SUNOS-{HOWTO,NOTES}.md` |
