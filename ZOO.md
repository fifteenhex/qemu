...
| `sun3x` (Sun 3/80) | 68030 (68851) | ✅ Linux shell · ⧗ **genuine 3/80 PROM boots real SunOS 4.1.1: root mounts off the UFS miniroot (`root on sd6a`), the kernel recognizes the Sun-3/80, prints its ethernet address, and enters device autoconfiguration** (`sm0 at obio`, `st0-3`/`sr0` SCSI probes) — **the kernel completes FULL device autoconfiguration** (`sm0 st0-3 sr0 sd0-sd6 fdc0 fd0 zs0 zs1 le0`), no crash/panic; walled at M8 — **the PMMU-validation payoff**: the kernel-memory allocator spins because QEMU's softmmu TLB cannot represent SunOS 4.1.1's kernelmap page frames (8 KB pages aligned to only 256 B; QEMU's `TARGET_PAGE_BITS_MIN=9`/512 B rounds them inconsistently). Reaching a shell needs a core-QEMU softmmu change, not sun3x device work (which is complete through full autoconfig). Fixed en route: sd-open SCSI handshake, an 82072 FDC responder, a memory-error-register status mask, and a real level-7-NMI storm | `hw/m68k/sun3x-NOTES.md`, `hw/m68k/sun3x-SUNOS-{HOWTO,NOTES}.md` |
